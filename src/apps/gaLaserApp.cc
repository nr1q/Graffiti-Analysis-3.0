...
{
    if (bSetup) {
        ofRemoveListener( ofEvents().mouseMoved,    this, &GaLaserApp::mouseMoved );
        ofRemoveListener( ofEvents().mousePressed,  this, &GaLaserApp::mousePressed );
        ofRemoveListener( ofEvents().mouseReleased, this, &GaLaserApp::mouseReleased );
        ofRemoveListener( ofEvents().mouseDragged,  this, &GaLaserApp::mouseDragged );
        ofRemoveListener( ofEvents().keyPressed,    this, &GaLaserApp::keyPressed );
        ofRemoveListener( ofEvents().keyReleased,   this, &GaLaserApp::keyReleased );
    }
}

//--------------------------------------------------------------
void GaLaserApp::setup()
{
    // laser specific
    smoothX = 0.0;
    smoothY = 0.0;
    ...
    ofAddListener( ofEvents().mouseMoved,    this, &GaLaserApp::mouseMoved );
    ofAddListener( ofEvents().mousePressed,  this, &GaLaserApp::mousePressed );
    ofAddListener( ofEvents().mouseReleased, this, &GaLaserApp::mouseReleased );
    ofAddListener( ofEvents().mouseDragged,  this, &GaLaserApp::mouseDragged );
    ofAddListener( ofEvents().keyPressed,    this, &GaLaserApp::keyPressed );
    ofAddListener( ofEvents().keyReleased,   this, &GaLaserApp::keyReleased );
...
//--------------------------------------------------------------
void GaLaserApp::updateParticles(){
...
//--------------------------------------------------------------
void GaLaserApp::updateTransition( int type)
{
        //----------  TRANSITIONS
        ...
        //{
                ...
                //{
                        ...
                        //{
                                ...
                                //ofVec3f pPos = ofVec3f(particleDrawer.PS.pos[randomP][0],particleDrawer.PS.pos[randomP][1],particleDrawer.PS.pos[randomP][2]);
                                //ofVec3f pVel = ofVec3f(particleDrawer.PS.vel[randomP][0],particleDrawer.PS.vel[randomP][1],particleDrawer.PS.vel[randomP][2]);
                ...
                for(int k = 0; k < simpleLine.size(); k++){
                        ...
                        for(int i = 0; i < simpleLine[k].size(); i++){
                                ...
                                if( i > 0 && i < simpleLine[k].size() -1 ){
                                        ...
                                        ofVec2f diff = simpleLine[k].at(i)-simpleLine[k].at(i-1);
                                        diff.x *= screenW;
                                        diff.y *= screenH;
                                        diff.normalize();
...
void GaLaserApp::keyPressed(ofKeyEventArgs & event)
{
    if (! bEnabled) return;
}

void GaLaserApp::keyReleased(ofKeyEventArgs & event)
{
    if (! bEnabled) return;
}

void GaLaserApp::mouseMoved(ofMouseEventArgs & event )
{
    if (! bEnabled) return;
}

void GaLaserApp::mouseDragged(ofMouseEventArgs & event )
{
    if (! bEnabled) return;
...
void GaLaserApp::mousePressed(ofMouseEventArgs & event)
{
    if (! bEnabled) return;

    if (bShowPanel)
        panel.mousePressed(event.x, event.y, event.button);
}

void GaLaserApp::mouseReleased(ofMouseEventArgs & event)
{
    if (! bEnabled) return;
...
//--------------------------------------------------------------
void GaLaserApp::setupControlPanel()
{
...
//--------------------------------------------------------------
void GaLaserApp::updateControlPanel(bool bUpdateAll)
{
    panel.update();
    panel.clearAllChanged();
        ...
        if (bUseGlobalZ) {
            ...
            if (tags.size() > 0 &&
                    (lastUpdatedZ != currentTagID ||
                     tags[currentTagID].z_const != z_const)
               ) {
...
//--------------------------------------------------------------
bool GaLaserApp::checkLaserActive()
{
    if (panel.hasValueChanged("useLaser") ||
        panel.hasValueChanged("bCamera")
        ) {
...
//--------------------------------------------------------------
void GaLaserApp::handleLaserPlayback(){
        ...
        laserTracker.processFrame(panel.getValueF("hue"),
                                  panel.getValueF("hueWidth"),
                                  panel.getValueF("saturation"),
                                  panel.getValueF("value"),
                                  2, 12, 0.1, true);
...
//--------------------------------------------------------------
void GaLaserApp::handleLaserRecord(){
        ...
        bool newTag = ( panel.hasValueChanged("laserMode") &&
                        panel.getValueB("laserMode") );
        ...
        if( laserTracker.newData() ){
                ...
                if( bNewStroke ){
                        ...
                        tags.back().nextStroke();
                        tags.back().addNewPoint(ofVec3f(smoothX, smoothY) );

                        simpleLine.push_back( simpleStroke() );
                        simpleLine.back().push_back(ofVec3f(smoothX, smoothY) );
...
//--------------------------------------------------------------
void GaLaserApp::checkLaserHitState(){
...
//--------------------------------------------------------------
void GaLaserApp::resetPlayer(int next)
{
        ...
        //tags[currentTagID].rotation = 0.0; // original
        tags[currentTagID].rotation = ofVec3f(0.0);
...
//--------------------------------------------------------------
void GaLaserApp::nextTag(int dir)
{
...
//--------------------------------------------------------------
string GaLaserApp::getTimeString()
{
