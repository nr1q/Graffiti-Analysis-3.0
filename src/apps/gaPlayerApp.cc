...
GaPlayerApp::~GaPlayerApp()
{
    ofRemoveListener( ofEvents().mouseMoved,    this, &GaPlayerApp::mouseMoved );
    ofRemoveListener( ofEvents().mousePressed,  this, &GaPlayerApp::mousePressed );
    ofRemoveListener( ofEvents().mouseReleased, this, &GaPlayerApp::mouseReleased );
    ofRemoveListener( ofEvents().mouseDragged,  this, &GaPlayerApp::mouseDragged );
    ofRemoveListener( ofEvents().keyPressed,    this, &GaPlayerApp::keyPressed );
...
void GaPlayerApp::updateParticles()
{
    ...
    particleDrawer.update(
        myTagPlayer.getCurrentPoint(),
        myTagPlayer.getVelocityForTime(&tags[currentTagID]),
        dt,
        myTagPlayer.bReset,
        tags[currentTagID].center.z
    );
...
void GaPlayerApp::updateTransition (int type)
{
        //----------  TRANSITIONS
...
void GaPlayerApp::updateArchitecture()
{
...
//--------------------------------------------------------------
void GaPlayerApp::drawTagNormal()
{
    fbo.clear();
    fbo.allocate(screenW, screenH);
    fbo.begin();
    ...
    if (mode == PLAY_MODE_PLAY && tags.size() > 0) {
        ...
        // data
        ofSetColor(255, 255, 255, 255);
        if (bShowName && tags.size() > 0)
            fontL.drawString( tags[ currentTagID ].tagname, 10,ofGetHeight()-30 );
...
//--------------------------------------------------------------
void GaPlayerApp::drawStereoEye(float eyeDist)
{
    ofEnableAlphaBlending();
    ofSetColor(255,255,255,255);
...
//--------------------------------------------------------------
void GaPlayerApp::keyPressed (ofKeyEventArgs& event)
{
...
//--------------------------------------------------------------
void GaPlayerApp::keyReleased (ofKeyEventArgs & event)
{
...
//--------------------------------------------------------------
void GaPlayerApp::mouseMoved(ofMouseEventArgs & event ){
    lastX = event.x;
    lastY = event.y;
}
...
//--------------------------------------------------------------
void GaPlayerApp::mouseDragged (ofMouseEventArgs & event)
{
    if( bShowPanel )
    {
            if(!panel.minimize){
...
//--------------------------------------------------------------
void GaPlayerApp::mousePressed (ofMouseEventArgs& event)
{
...
//--------------------------------------------------------------
void GaPlayerApp::mouseReleased (ofMouseEventArgs& event)
{
...
//--------------------------------------------------------------
void GaPlayerApp::resetPlayer (int next)
{
...
//--------------------------------------------------------------
void GaPlayerApp::nextTag (int dir)
{
    if (dir == 1) {
        currentTagID++;
...
//--------------------------------------------------------------
void GaPlayerApp::loadTags()
{
    if (threadedLoader.bResponseReady) {
        if (
            threadedLoader.getResponse() == 1 &&
            threadedLoader.totalLoaded > 0 &&
            tags.size() < (unsigned)threadedLoader.totalToLoad
        ) {
            loadStatus = "Loading...";
            if (tags.size() < (unsigned)threadedLoader.totalLoaded) {
                grafTagMulti temptTag;
                tags.push_back( temptTag );
                int toLoad = tags.size()-1;
                tags[ toLoad ] = threadedLoader.tags[ toLoad ];
                //smoother.smoothTag(4, &tags[ toLoad ]); // uncomment this
                tags[toLoad].average();
                tags[toLoad].average();
            }
        ...
        //std::cout << " threadedLoader totalToLoad: " << threadedLoader.totalToLoad << std::endl;
    }
...
//--------------------------------------------------------------
void GaPlayerApp::preLoadTags (int myMode)
{
    if (myMode == 0) {
...
//--------------------------------------------------------------
void GaPlayerApp::saveAllTagPositions()
{
...
//--------------------------------------------------------------
void GaPlayerApp::saveTagPositions()
{
    if(currentTagID < 0) return;
    if( !threadedLoader.bResponseReady ) return;
...
//--------------------------------------------------------------
void GaPlayerApp::setupControlPanel()
{
...
//--------------------------------------------------------------
void GaPlayerApp::updateControlPanel(bool bUpdateAll)
{
    panel.update();
    keypanel.update();
    ...
    if( panel.getSelectedPanelName() == "Draw Settings" || bUpdateAll)
    {
        ...
        if (bUseGlobalZ) {
            ...
            if (
                tags.size() > 0 &&
                (lastUpdatedZ != currentTagID || tags[currentTagID].z_const != z_const)
            ) {
...
string GaPlayerApp::getCurrentTagName()
{
    if (tags.size() <= 0)
        return " ";
    else if ( mode == PLAY_MODE_LOAD)
        return tags[tags.size()-1].tagname;
    else
        return tags[currentTagID].tagname;
}
...
void GaPlayerApp::loadScreenSettings()
{
    ofxXmlSettings xml;
...
void GaPlayerApp::updateZDepth(float z_const)
{
