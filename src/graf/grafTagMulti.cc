...
bool grafTagMulti::addNewPoint (ofVec3f pt, float dist, float angle, float time, bool bAverage)
{
    //----- ignore 0,0 error
    if (pt.x == 0 && pt.y == 0) {
        cout << "pt bad " << endl;
    ...
    if (! bPtsEmpty && dist == -1)
        dist = ( (ofVec2f)(myStrokes[ id ].pts[ lastPt ].pos-pt) ).length();
    else if (bPtsEmpty && dist == -1)
        dist = 0;

    if (bPtsEmpty || dist > minPtDist) {
        myPoint.dist = dist;
    }
    else {
    ...
    if (! bPtsEmpty && angle == -1)
        angle = atan2(pt.y - myStrokes[ id ].pts[lastPt].pos.y,
                      pt.x - myStrokes[ id ].pts[lastPt].pos.x);
    else if (angle == -1)
        angle = 0;
    ...
    int inPt = -1;
    for (unsigned int i = 0; i < myStrokes[ id ].pts.size(); i++) {
...
{
    for (unsigned int i = 0; i < myStrokes.size(); i++)
...
{
    for (unsigned int i = 2; i < stroke->pts.size(); i++) {
...
void grafTagMulti::calcMinMax()
{
    ...
    for (unsigned int i = 0; i < myStrokes.size(); i++) {
        for (unsigned int j = 0; j < myStrokes[i].pts.size(); j++) {
...
void grafTagMulti::drawBoundingBox (ofVec3f pmin, ofVec3f pmax, ofVec3f pcenter)
{
    // draw cube
    float pos[18];
