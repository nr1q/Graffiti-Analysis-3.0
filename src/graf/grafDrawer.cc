...
void grafDrawer::transitionDeform (float dt, float pct, float* amps, int numAmps)
{
    ...
    for (unsigned int i = 0; i < lines.size(); i++) {
        for (unsigned int j = 0; j < lines[i]->pts_l.size(); j++) {
...
void grafDrawer::draw (int lastStroke, int lastPoint)
{
    ...
    for (unsigned int i = 0; i < lines.size(); i++) {
        if (i < (unsigned)lastStroke)
            lines[i]->draw(-1,alpha,-1,lineWidth);
        else if (i == (unsigned)lastStroke)
            lines[i]->draw(lastPoint,alpha,-1,lineWidth);

        //if (i < lastStroke)
            //lines[i]->drawOutline(-1,alpha,lineWidth);
        //else if (i == lastStroke)
            //lines[i]->drawOutline(lastPoint,alpha,lineWidth);
    }
...
void grafDrawer::drawTimeLine (ofVec3f center, float currentTime, float startTime, float z_const, ofTrueTypeFont * font, float scale)
{
...
void grafDrawer::drawBoundingBox (ofVec3f pmin, ofVec3f pmax, ofVec3f pcenter)
{
    glColor4f(1, 1, 1, 1);
