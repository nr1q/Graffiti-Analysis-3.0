...
void grafPlayer::calcPointForTime (float time, grafTagMulti* myTag)
{
    ...
    for (unsigned int i = 0; i < myTag->myStrokes.size(); i++) {
        for (unsigned int j = 0; j < myTag->myStrokes[i].pts.size(); j++) {
    ...
    if (whatPointAmINear > 0) {
        ...
        ofVec3f pta = myTag->myStrokes[whatStrokeAmIn].pts[whatPointAmINear - 1].pos;
        float timeb = myTag->myStrokes[whatStrokeAmIn].pts[whatPointAmINear    ].time;
        ofVec3f ptb = myTag->myStrokes[whatStrokeAmIn].pts[whatPointAmINear    ].pos;
...
{
    ofDrawRectangle( nowPointForTime.x*scale, nowPointForTime.y*scale, 2, 2 );
}


ofVec3f grafPlayer::getVelocityForTime (grafTagMulti* myTag)
{
    ...
    //----- don't do it if there are no points
    if (myTag->myStrokes.size() == 0)        return ofVec3f(0, 0, 0);
    if (myTag->myStrokes[0].pts.size() == 0) return ofVec3f(0, 0, 0);

    //----- get the total time of the tag
    //float totalTime = getDuration(myTag); // unused
    ...
    ofVec3f prevPoint = nowPointForTime;

    calcPointForTime(timeCounter, myTag);
    ofVec3f curPoint = nowPointForTime;
...
/*
ofVec3f grafPlayer::getPointForTime(    float time, int & whatPointAmINear, float & dist, float & angle)
{

        if (pts.size() <= 1) return ofVec3f(0,0,0);
    ...
    timeInRange         = time;
        ...
        if (whatPointAmINear > 0){
                ...
                ofVec3f pta = pts[whatPointAmINear - 1].pos;
                float timeb = pts[whatPointAmINear    ].time;
                ofVec3f ptb = pts[whatPointAmINear    ].pos;
...
ofVec3f grafPlayer::getPointForTime( float time)
{
    ...
    float myTimeInRange   = time;
        ...
        if (whatAmINear > 0){
                ...
                ofVec3f pta = pts[whatAmINear - 1].pos;
                float timeb = pts[whatAmINear    ].time;
                ofVec3f ptb = pts[whatAmINear    ].pos;
...
ofVec3f grafPlayer::getCurrentPoint()
{
