...
void GaThreadedLoader::start()
{
    if (isThreadRunning() == false) {
        ...
        startThread(false); // blocking = true
    }
}
...
void GaThreadedLoader::getDirectoryInfo(string myTagDirectory)
{
    ...
    dirLister.listDir( ofToDataPath(myTagDirectory, true) );
    dirLister.allowExt( "gml" );
    ...
    //}
    for (unsigned i = 0; i < dirLister.size(); ++i) {
        file = dirLister.getFile(i);
        filesToLoad.push_back( dirLister.getPath(i) );
        filenames.push_back( file.getBaseName() );
    }
...
{
    if (bResponseReady) {
        if (bUseRss && !bDownloadedFiles)
            return 0;
        else if ( bHaveDirs && totalToLoad > 0 )
            return 1;
        else
            return 0;
    }
