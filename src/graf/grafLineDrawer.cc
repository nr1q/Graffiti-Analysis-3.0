...
void grafLineDrawer::setup (vector<timePt> pts, float minLen, float maxLen, float blendPct, int start, int end, bool bFadeIn, bool bFadeOut)
{
    ...
    //ofVec2f ppPrev = ofVec2f(1,1); // unused
    bool bSwap = false;
    ...
    for (int i = start; i < end; i++) {
        ...
        if (! bSwap) {
            ...
            pts_l.push_back( ofVec3f( pts[i].pos.x-( len*sin(pts[i].angle) ),pts[i].pos.y+( len*cos(pts[i].angle) ), pts[i].pos.z) );
            pts_r.push_back( ofVec3f( pts[i].pos.x+( len*sin(pts[i].angle) ),pts[i].pos.y-( len*cos(pts[i].angle) ), pts[i].pos.z)  );

            float oLen = len+outlineDist;
            pts_lout.push_back( ofVec3f( pts[i].pos.x-( oLen*sin(pts[i].angle) ),pts[i].pos.y+( oLen*cos(pts[i].angle) ), pts[i].pos.z) );
            pts_rout.push_back( ofVec3f( pts[i].pos.x+( oLen*sin(pts[i].angle) ),pts[i].pos.y-( oLen*cos(pts[i].angle) ), pts[i].pos.z)  );
        } else {
        ...
        ofVec2f vec = ofVec2f( pts_r[pts_r.size()-1].x-pts_l[pts_l.size()-1].x,pts_r[pts_r.size()-1].y-pts_l[pts_l.size()-1].y );
        vec = vec.normalize();
        vecs.push_back(vec);
    }
}
...
void grafLineDrawer::setupUniform (vector<timePt> pts, float lineDist, int start, int end)
{
    ...
    for (int i = start; i < end; i++) {
        ...
        if (deltaA > (HALF_PI) && TWO_PI-deltaA > HALF_PI)
            bSwap = !bSwap;
...
void grafLineDrawer::calculatePoint (timePt pt, float time_num, float dist, float angle, vector<ofVec3f>& left, vector<ofVec3f>& right)
{
    dist = MAX( dist ,.01 );
    ...
    left.push_back( ofVec3f(left_x, left_y, time_num) );
    right.push_back( ofVec3f(right_x, right_y, time_num) );
}
...
void grafLineDrawer::average (float pct)
{
    ...
    for (unsigned int i = 1; i < pts_l.size(); i++) {
        if (i >= 2) {
    ...
    for (unsigned int i = 1; i < pts_r.size(); i++) {
        if (i >= 2) {
    ...
    for (unsigned int i = 0; i < pts_r.size(); i++) {
