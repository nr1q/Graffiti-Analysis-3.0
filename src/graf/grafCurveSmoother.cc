...
void grafCurveSmoother::smoothTag( int resolution, grafTagMulti* src )
{
    ...
    for (unsigned int i = 0; i < src->myStrokes.size(); i++) {
    ...
    for (unsigned int i = 0; i < tempTag.myStrokes.size(); i++) {
...
void grafCurveSmoother::smoothStroke(int resolution, strokes* src)
{
    ...
    for (unsigned int j = 4;j < src->pts.size(); j+=1) {
    ...
    sort(
        tempTag.myStrokes[stk].pts.begin(),
        tempTag.myStrokes[stk].pts.end(),
        sort_zpos_compare
    );
    tempTag.nextStroke();
}
