...
ofxControlPanel::~ofxControlPanel()
{
    for (int i = 0; i < (signed)guiObjects.size(); i++)
        delete guiObjects[i];
}
...
{
    guiTTFFont.load( fontName, fontsize, true, true );
...
{
    if (whichPanel < 0 || (unsigned)whichPanel >= panels.size())
        return;
...
void ofxControlPanel::setWhichColumn(int column){
    if (currentPanel < 0 || (unsigned)currentPanel >= panels.size())
        return;
...
{
    if( currentPanel < 0 || (unsigned)currentPanel >= panels.size() )return NULL;
...
{
    for (unsigned i = 0; i < guiObjects.size(); i++) {
...
bool ofxControlPanel::isAnyTextBoxActive()
{
    for (unsigned int i = 0; i < xmlObjects.size(); i++) {
        if (
            xmlObjects[i].guiObj->dataType == SG_TYPE_STRING &&
            xmlObjects[i].guiObj->value.getValueB()
        )
            return true;
    }
    return false;
