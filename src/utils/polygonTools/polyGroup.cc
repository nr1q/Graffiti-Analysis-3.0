...
{
    for (unsigned int i = 0; i < polys.size(); i++) {
        if (i == (unsigned)selectedId && polys[i]->bClosed) {
            ofRectangle rect = polys[i]->getBoundingBox();
            ofNoFill();
            ofSetColor(100, 100, 100, 255);
            ofDrawRectangle( rect.x-4, rect.y-4, rect.width+8, rect.height+8 );
        }

        if (i == (unsigned)selectedId)
            ofSetColor(strokeColor.r, strokeColor.g, strokeColor.b, strokeColor.a);
        else
            ofSetColor(strokeColor.r, strokeColor.g, strokeColor.b, strokeColor.a*.75);
...
void polyGroup::keyPressed (ofKeyEventArgs& event)
{
    ...
    if (event.key == OF_KEY_BACKSPACE) {
        if (polys.size() > (unsigned)selectedId) {
