...
void polyEditable::addPoint (ofVec3f pt)
{
    //cout << "add point " << endl;
    if (bUseClosePoly && pts.size() > 2) {
        if (
            abs( (int)(pts[0].x - pt.x) ) <= selectDist &&
            abs( (int)(pts[0].y - pt.y) ) <= selectDist
        ) {
            bClosed = true;
        }
    }
...
void polyEditable::removePoint()
{
    ...
    if (pts.size() > (unsigned)selectedPoint) {
...
void polyEditable::keyPressed (ofKeyEventArgs& event)
{
    ...
    if (event.key == OF_KEY_UP) {
        ofVec2f mup = ofVec2f( 0, -moveBy );
        mup.rotate( -gRotation );
        if (mode == POLY_EDIT_MODE_MOVE_ALL)
            moveAllPointsBy( ofVec3f(mup.x, mup.y) ); //ofVec3f(0,-1) );
        else if (mode == POLY_EDIT_MODE_MOVE_PTS)
            movePointBy( selectedPoint, ofVec3f(mup.x, mup.y) ); //ofVec3f(0,-1) );
    } else if (event.key == OF_KEY_DOWN) {
        ofVec2f mdwn = ofVec2f( 0, moveBy );
        mdwn.rotate( -gRotation );
        if (mode == POLY_EDIT_MODE_MOVE_ALL)
            moveAllPointsBy( ofVec3f(mdwn.x, mdwn.y) );
        else if (mode == POLY_EDIT_MODE_MOVE_PTS)
            movePointBy( selectedPoint, ofVec3f(mdwn.x, mdwn.y) );
    } else if (event.key == OF_KEY_LEFT) {
        ofVec2f m = ofVec2f( -moveBy, 0 );
        m.rotate( -gRotation );
        if (mode == POLY_EDIT_MODE_MOVE_ALL)
            moveAllPointsBy( ofVec3f(m.x, m.y) );
        else if (mode == POLY_EDIT_MODE_MOVE_PTS)
            movePointBy( selectedPoint, ofVec3f(m.x, m.y) );
...
ofVec3f polyEditable::getMouseAltered (ofVec3f m)
{
    ofVec2f mpt = ofVec2f( m.x - transPreRotate.x, m.y - transPreRotate.y );
...
void polyEditable::mouseDragged(ofMouseEventArgs& event)
{
    ...
    ofVec3f m = getMouseAltered( ofVec3f(event.x, event.y) );

    if (mode == POLY_EDIT_MODE_MOVE_PTS) {
        if (selectedPoint >= 0)
            pts[ selectedPoint ].set( m.x, m.y );
    } else if (mode == POLY_EDIT_MODE_MOVE_ALL) {
        ofVec3f diff = ofVec3f( (m.x-lastCenter.x)-moveOffSet.x, (m.y-lastCenter.y)-moveOffSet.y);
...
void polyEditable::draw (bool bDrawClosed)
{
    ...
    if (selectedPoint >= 0) {
            ofNoFill();
            if (pts.size() > (unsigned)selectedPoint)
                ofDrawCircle(pts[selectedPoint].x, pts[selectedPoint].y, 4);
            if (mode == POLY_EDIT_MODE_MOVE_PTS) {
                ofFill();
                ofSetColor(255, 255, 255, 255);
                if (pts.size() > (unsigned)selectedPoint)
                    ofDrawCircle(pts[selectedPoint].x, pts[selectedPoint].y, 1);
                ofNoFill();
...
void polyEditable::moveAllPointsBy (ofVec3f pt)
{
    for (unsigned int i = 0; i < pts.size(); i++) {
        pts[i].x += pt.x;
        pts[i].y += pt.y;
...
void polyEditable::movePointBy (int index, ofVec3f pt)
{
    if ((unsigned)index < pts.size() && index >= 0) {
        pts[index].x += pt.x;
        pts[index].y += pt.y;
    }
}
...
void polyEditable::rotate (float r)
{
    ...
    for (unsigned int i = 0; i < pts.size(); i++) {
        ofVec2f vPt = ofVec2f( pts[i].x, pts[i].y );
        vPt.rotate( r, cpt );
...
void polyEditable::setOffset (ofVec3f preR, ofVec3f pstR)
{
