...
void polySimple::pushVertex (ofVec3f pt)
{
    pts.push_back( pt );
}


void polySimple::popVertex (ofVec3f pt)
{
    if (pts.size() > 0)
        pts.pop_back();
}
...
void polySimple::insertVertex (ofVec3f pt, int ptBefore)
{
    if (pts.size() > (unsigned)ptBefore)
...
void polySimple::draw()
{
    ...
    if (bDrawnWithPoints) {
        ...
        for (unsigned int i = 0; i < pts.size(); i++)
            ofDrawCircle( pts[i].x, pts[i].y, 1 );
    }
}
...
void polySimple::setWinding (int direction)
{
    ...
    if (
        (bCounterClockwise && direction==WIND_CLOCKWISE) ||
        (! bCounterClockwise && direction==WIND_COUNTER_CLOCKWISE)
    ) {
        vector<ofVec3f> temp;
...
ofVec3f polySimple::getCentroid()
{
    float cx=0;
    float cy=0;
...
ofVec3f polySimple::getPt (int id)
{
    if ((unsigned)id < pts.size())
        return pts[id];
    else
        return pts[0];
}
