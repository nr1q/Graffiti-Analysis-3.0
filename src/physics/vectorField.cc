...
//===============================================================
void vectorField::update (ofVec3f pt, ofVec3f vel, float scale,  float w, float h)
{
    if (vel.x == 0 && vel.y == 0) {
        lastx = pt.x * scale;
        lasty = pt.y * scale;
...
void vectorField::addToFieldAnSpecialCircleShape (float x, float y, float vx, float vy, float radius, float strength)
{
    ...
    for (unsigned int i = 0; i < WIDTH_FIELD; i++) {
        for (unsigned int j = 0; j < HEIGHT_FIELD; j++) {
            float distanceSq = (fieldPosX-i) * (fieldPosX-i) +
...
void vectorField::drawField (float width, float height)
{
    ...
    for (unsigned int i = 0; i < WIDTH_FIELD; i+=2) {
        for (unsigned int j = 0; j < HEIGHT_FIELD; j+=2) {
            // pos in array
...
//===============================================================
ofVec3f vectorField::getVel (float xpos, float ypos)
{
    int fieldPosX = (int)(xpos * (float)WIDTH_FIELD);
    int fieldPosY = (int)(ypos * (float)HEIGHT_FIELD);

    if (
        fieldPosX < 0 || fieldPosX > WIDTH_FIELD-1 ||
